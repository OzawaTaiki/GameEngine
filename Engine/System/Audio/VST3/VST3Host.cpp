...
VST3Host* Engine::VST3Host::GetInstance()
{
...
bool Engine::VST3Host::Initialize([[maybe_unused]]const std::string& hostName)
{
...
void Engine::VST3Host::Finalize()
{
    modules_.clear();
    hostApp_.reset();
}

VST3Module* Engine::VST3Host::LoadModule(const std::string& path)
{
...
void Engine::VST3Host::UnloadModule(const std::string& path)
{
    modules_.erase(path);
}

Steinberg::Vst::HostApplication* Engine::VST3Host::GetHostApp()
{
    return hostApp_.get();
}
