...
std::wstring ShaderCompiler::MakeCacheKey(const std::wstring &_filePath,
...
ShaderCompiler::Compile(const std::wstring &_filePath, const wchar_t *_profile,
                        const std::wstring &_entryFuncName,
                        const std::wstring &_dirPath) {

  // キャッシュチェック
  std::wstring cacheKey =
      MakeCacheKey(_filePath, _profile, _entryFuncName, _dirPath);
  ...
  Debug::Log(ConvertString(std::format(
  ...
  LPCWSTR arguments[] = {
      _filePath.c_str(), L"-E",  _entryFuncName.c_str(), L"-T",
      _profile,          L"-Zi", L"-Qembed_debug",       L"-Od",
      L"-Zpr",
  };

  Microsoft::WRL::ComPtr<IDxcResult> shaderResult = nullptr;
  hr =
      dxcCompiler_->Compile(&shaderSourceBuffer, arguments, _countof(arguments),
...
void ShaderCompiler::Register(const std::string &_name,
...
Microsoft::WRL::ComPtr<IDxcBlob> ShaderCompiler::Get(const std::string &_name) {
  ...
  return Compile(info.filePath, info.profile.c_str(), info.entryFuncName,
                 info.dirPath);
}
...
void ShaderCompiler::RegisterCommonShaders() {
  ...
  Register("FullScreen_PS", L"FullScreen.PS.hlsl", L"ps_6_0");
  Register("Composite_PS", L"Composite.hlsl", L"ps_6_0");
  ...
  // ライン描画用
  Register("Line_VS", L"LineDrawer.VS.hlsl", L"vs_6_0");
  Register("Line_PS", L"LineDrawer.PS.hlsl", L"ps_6_0");
  ...
  // DLShadowMap用
  Register("DLShadowMap_VS", L"ShadowMap.hlsl", L"vs_6_0", L"ShadowMapVS");
  Register("DLShadowMap_PS", L"ShadowMap.hlsl", L"ps_6_0", L"ShadowMapPS");

  // PLShadowMap用
  Register("PLShadowMap_VS", L"PointLightShadowMap.hlsl", L"vs_6_0", L"VSmain");
  Register("PLShadowMap_GS", L"PointLightShadowMap.hlsl", L"gs_6_0", L"GSmain");
  Register("PLShadowMap_PS", L"PointLightShadowMap.hlsl", L"ps_6_0", L"PSmain");
